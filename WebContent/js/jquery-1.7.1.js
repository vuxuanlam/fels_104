...
(function(window, undefined) {
	...
	jQuery.support = (function() {
		...
		jQuery(function() {
			...
			html = "<div " + style + "><div></div></div>" + "<table " + style
								...
								callback
										.call(
												table ? root(this[i], first)
														: this[i],
												// Make sure that we do not leak
												// memory by inadvertently
												// discarding
												// the original fragment (which
												// might have attached data)
												// instead of
												// using it; in addition, use
												// the original fragment object
												// for the last
												// item instead of first because
												// it can end up being emptied
												// incorrectly
												// in certain situations (Bug
												// #8070).
												// Fragments from the fragment
												// cache must always be cloned
												// and never used
												// in place.
												results.cacheable
			...
			for (type in events) {
				for (i = 0, l = events[type].length; i < l; i++) {
					jQuery.event.add(dest, type
							...
							+ events[type][i].namespace, events[type][i],
							events[type][i].data);
				}
			}
		}
	...
	// Derived From:
	// http://www.iecss.com/shimprove/javascript/shimprove.1-0-1.js
	...
	jQuery
			.extend({
				clone : function(elem, dataAndEvents, deepDataAndEvents) {
					...
					// IE<=8 does not properly clone detached, unknown element
					// nodes
					clone = jQuery.support.html5Clone
							|| !rnoshimcache.test("<" + elem.nodeName) ? elem
							.cloneNode(true) : shimCloneNode(elem);
					...
					for (var i = 0, elem; (elem = elems[i]) != null; i++) {
						...
						if (typeof elem === "string") {
							if (!rhtml.test(elem)) {
								...
								// Append wrapper element to unknown element
								// safe doc fragment
								if (context === document) {
									// Use the fragment we've already created
									// for this document
									safeFragment.appendChild(div);
								} else {
									// Use a fragment created with the owner
									// document
					...
					for (var i = 0, elem; (elem = elems[i]) != null; i++) {
						...
						if (id) {
							...
							if (data && data.events) {
								...
								// Null the DOM reference to avoid IE6/7/8 leak
								// (#7054)
								if (data.handle) {
	...
	function evalScript(i, elem) {
		if (elem.src) {
			...
			jQuery
					.globalEval((elem.text || elem.textContent
	...
	rupper = /([A-Z]|^ms)/g, rnumpx = /^-?\d+(?:px)?$/i, rnum = /^-?\d/, rrelNum = /^([\-+])=([\-+.\de]+)/,
	...
	function getWH(elem, name, extra) {
		...
		var val = name === "width" ? elem.offsetWidth : elem.offsetHeight, which = name === "width" ? cssWidth
				: cssHeight, i = 0, len = which.length;
				...
				for (; i < len; i++) {
					if (!extra) {
						val -= parseFloat(jQuery
								.css(elem, "padding" + which[i])) || 0;
					}
					if (extra === "margin") {
						val += parseFloat(jQuery.css(elem, extra + which[i])) || 0;
					} else {
						val -= parseFloat(jQuery.css(elem, "border" + which[i]
								+ "Width")) || 0;
					}
				}
			}
	...
	function inspectPrefiltersOrTransports(structure, options, originalOptions,
			jqXHR, dataType /* internal */, inspected /* internal */) {
				...
				load : function(url, params, callback) {
					...
					jQuery.ajax({
						...
						complete : function(jqXHR, status, responseText) {
							// Store the response as specified by the jqXHR
							// object
	...
	jQuery
			.extend({
				...
				ajaxSettings : {
					 ...
					 * password: null, cache: null, traditional: false, headers:
					 * {},
					 */
					...
					if (!s.crossDomain || jQuery.support.cors) {
						...
						return {
							send : function(headers, complete) {
								...
								// Open the socket
								// Passing null username, generates a login
								// popup on Opera (#2865)
								if (s.username) {
								...
								callback = function(_, isAbort) {
									...
									try {
										...
										// Was never called and is aborted or
										// complete
										if (callback
											...
											if (isAbort) {
												...
												// If the request is local and
												// we have data: assume a
												// success
												// (success with no data won't
												// get notified, that's the best
												// we
				...
				animate : function(prop, speed, easing, callback) {
					...
					function doAnimation() {
						...
						var opt = jQuery.extend({}, optall), isElement = this.nodeType === 1, hidden = isElement
								&& jQuery(this).is(":hidden"), name, val, p, e, parts, start, end, unit, method;
	...
	jQuery.fx.prototype = {
		...
		show : function() {
			...
			if (dataShow !== undefined) {
				// This show is picking up where a previous hide or show left
				// off
		...
		step : function(gotoEnd) {
			...
			if (gotoEnd || t >= options.duration + this.startTime) {
				...
				if (done) {
					...
					// in the event that the complete function throws an
					// exception
		...
		if (!elemdisplay[nodeName]) {
			...
			if (display === "none" || display === "") {
				...
				if (!iframeDoc || !iframe.createElement) {
					...
					iframeDoc
							.write((document.compatMode === "CSS1Compat" ? "<!doctype html>"
									: "")
									+ "<html><body>");
					iframeDoc.close();
				}
						...
						jQuery.fn[type] = function(size) {
							...
							if (jQuery.isFunction(size)) {
								return this.each(function(i) {
									var self = jQuery(this);
									self[type]
											(size.call(this, i, self[type]()));
								});
							}

							if (jQuery.isWindow(elem)) {
								// Everyone else use document.documentElement or
								// document.body depending on Quirks vs
								// Standards mode
								// 3rd condition allows Nokia support, as it
								// supports the docElem prop but not CSS1Compat
								var docElemProp = elem.document.documentElement["client"
										+ name], body = elem.document.body;
								return elem.document.compatMode === "CSS1Compat"
										&& docElemProp
										|| body
										&& body["client" + name] || docElemProp;

								// Get document width or height
							} else if (elem.nodeType === 9) {
								// Either scroll[Width/Height] or
								// offset[Width/Height], whichever is greater
								return Math.max(elem.documentElement["client"
										+ name], elem.body["scroll" + name],
